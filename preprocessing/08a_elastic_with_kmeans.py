...
if __name__ == "__main__":
    ...
    for i in range(n_features):
        kmean=KMeans(n_clusters=168, n_init=10, max_iter=300,
                     tol=0.0001,verbose=0, random_state=1, algorithm='auto')
        #{'alpha': 0.0001621985745106104, 'l1_ratio': 0.66, 'n_clusters': 168}
        #Minimum error is: 0.13378520645859335 for 1 features
        ssc=StandardScaler()
        pipe=Pipeline([
            ('kmean',kmean),
            ('ssc',ssc),
        ...
        print(f'Number of kmeans clusters: {i+1}','Mean RMSLE: %.4f (%.4f)' % (M, STD))
        results[i,0]=i+1
        results[i, 1] = M
        results[i, 2] = STD
        results[i, 3] = M+2*STD
    ...
    np.save("linear_errors.npy",results)
    df = pd.DataFrame(data=results, columns=["Clusters", "Mean","STD","Mean_2STD"])
    df.to_excel(
        'model_elastic_net_kmeans.xlsx',
        sheet_name='Linear',
        index=False)
